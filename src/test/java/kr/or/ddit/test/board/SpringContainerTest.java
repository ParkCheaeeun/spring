...
public class SpringContainerTest {
	...
	//name : 스프링 빈 이름
	//boardDao 라틑 이름의 스프링 빈을 스프링 컨테이너로부터 찾아 boardDao 필드에 주입
	...
	*/
	...
	@Resource(name="boardDaoS1")
	private IBoardDao boardDaoS_1;
	
	@Resource(name="boardDaoS2")
	private IBoardDao boardDaoS2;
	@Resource(name="boardDaoS2")
	private IBoardDao boardDaoS_2;
	...
	*/
	@Test
	public void springBeanScopTest() {
		/***Given***/
		...
		logger.debug("boardDaoS_1 : {}", boardDaoS_1);
		assertEquals(boardDaoS1, boardDaoS_1);
		logger.debug("boardDaoS2 : {}", boardDaoS2);
		logger.debug("boardDaoS_2 : {}", boardDaoS_2);
		assertEquals(boardDaoS2, boardDaoS_2);
		
		//디자인 패턴 관점에서 보면 boardDaoS1과 boardDaoS2는 같은 클래스로부터 생성 되었으므로 같아야하나,
		//스프링 컨테이너에서 관리되는 scope의 개념에 따라 다른 객체로 관리된다. 
		
		assertNotEquals(boardDaoS1, boardDaoS2);
		
		//boardDaoP, boardDaoP2는 boardDaoP라는 이름의 prototype scope를 갖는 스프링 빈, prototype 정의에 따라 두 필드에 주입된
		//객체는 서로 다른 객체이다.
